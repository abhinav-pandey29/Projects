...
def compute_edges_dxdy(I):
    ...
    # Apply gaussian blur to de-noise input image
    I = cv2.GaussianBlur(I, (7,7), 10)
    
    # Initialise the SCHARR kernel for edge detection
    ...
    # De-normalise gradient intensity to the range 0 to 255
    grad_intensity = grad_intensity / grad_intensity.max() * 255
    
    # Compute angle of gradient at each pixel
    angle = np.arctan2(dy, dx)
    
    # Initialise a zero matrix to store all strong non-maximum suppressed image
    Z = np.zeros((grad_intensity.shape))
    
    # Convolve over all the image pixels
    
    # Interpolation NMS code adapted from https://github.com/FienSoP/canny_edge_detector/blob/master/image_alignment_task.ipynb
    ...
    for i in range(grad_intensity.shape[0] - 1):
        for j in range(grad_intensity.shape[1] - 1):
            ...
            try:
                ...
                # Initialise values of neighboring pixels
                q, r = 255,255
